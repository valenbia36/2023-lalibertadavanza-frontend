...
const ResetPassword = () => {
    ...
    const handleResetPassword = async () => {
        if(password === repeatPassword){
            const response = await fetch('http://localhost:3001/api/auth/users/updatePassword/' + userId, {
                method: 'PUT',
                headers: {
            ...
            if(response.status === 200){
                enqueueSnackbar('The password was changed successfully.', { variant: 'success' });
                setIsValid(false);
                setToken('');
                setPassword('');
                setRepeatPassword('');
    ...
    const validateToken = async () => {
        const response = await fetch('http://localhost:3001/api/auth/users/' + token, {
            method: 'GET',
            headers: {
                "Content-Type": "application/json"
            }
  ...
  return (
    <div className="container">
        <h1><code>Reset your Password</code></h1>
        ...
        {!isValid && (
            <Grid>
                <TextField
                    ...
                    onKeyPress={(event) => { if (event.key === 'Enter') { validateToken() } }}
                />
